...
SELECT 
    ...
    SUM(s.SalesAmount) AS TotAm
FROM Sales s
JOIN Region r ON s.RegionID = r.RegionID
GROUP BY r.RegionName, EXTRACT(YEAR FROM s.SalesDate)
ORDER BY r.RegionName,Y DESC;
...
7. Rispondere alla seguente domanda: quali sono, se ci sono, i prodotti invenduti? Proponi due approcci risolutivi differenti. 
...
8. Esporre l’elenco dei prodotti con la rispettiva ultima data di vendita (la data di vendita più recente).
